...
1. **Run `review`** on the cumulative diff (baseline to final). Load the existing `review` skill on the optimization branch when the host exposes skills; otherwise perform an ordinary diff review. To land eligible fixes before the next option, apply the mechanical-apply bar below.
