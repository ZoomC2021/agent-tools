...
- **Idea mode:** the idea is a fixed given. Explain its implications, mechanics, and trade-offs for the user's understanding. Never scope it as requirements work, never generate and rank alternatives as product discovery.
...
- **New-capability ideas** — offer first; on acceptance invoke `to-prd` or continue with a focused product-discovery discussion using the observations as seed context.
- **Code-clarity findings** — offer first; on acceptance invoke `refactor` with the observations and the files they concern.
- **UI/UX polish opportunities** — present the observations in chat and offer `frontend-design` when the host exposes it.
