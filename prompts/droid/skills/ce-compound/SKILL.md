...
- Any code-heavy issue → preserve code simplification as a **read-only documentation review**. Inspect the solution draft's code examples and explanatory claims inline, or dispatch a generic subagent seeded with a local prompt only to return suggestions. Do **not** invoke a code-refactoring workflow from this phase and do not mutate product code unless the user explicitly asks for a separate code-simplification pass. Do not use the deleted `code-simplicity-reviewer`.
...
- **Manual trigger**: User can run existing skills such as `refactor` or `review` after `/ce-compound` completes for deeper code review and mutation

## Related Commands

- `/research [topic]` - Deep investigation (searches docs/solutions/ for patterns)
- Host planning workflow - references documented solutions
