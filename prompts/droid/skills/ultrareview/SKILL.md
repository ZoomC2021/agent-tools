...
Run a dual-model review with Droid-native subagents: the GPT-5.5 high-reasoning oracle droid and Gemini 3.1 Pro.

This mirrors the OpenCode UltraReview workflow but **must not** use Gemini CLI helpers or external CLI review processes.

## Models Used

1. `oracle` droid (`model: gpt-5.5`, `reasoningEffort: high`)
...
- `subagent_type: oracle` for GPT-5.5 high-reasoning review
