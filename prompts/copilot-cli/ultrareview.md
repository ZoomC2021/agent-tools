...
### Process 1: GPT Review (Copilot CLI)

**READ-ONLY REVIEW - Do NOT modify files during review**

Use Copilot CLI to review changes:
```bash
gh copilot suggest "Review these code changes for issues: $(git diff HEAD)"
```

Or use Copilot's explain feature on the diff.
...
- GPT/Copilot findings (from Copilot CLI)
- Gemini 3.1 Pro findings (from CLI output)

Handle failures gracefully:
- If Copilot fails → Use Gemini results alone
- If Gemini CLI fails → Use Copilot results alone
...
🔴 [<Original_Severity>] [Category] filename:line
   Consensus: Copilot GPT + Gemini 3.1 Pro
   Original Severity: <Critical/Warning/Suggestion>
   Problem: <description>
   Fix: <recommendation>
```

#### Section 2: 🟠 Copilot GPT Exclusive Findings
Issues found ONLY by Copilot GPT.
...
### Both Models Failure
Report: "UltraReview failed: Both Copilot GPT and Gemini 3.1 Pro unavailable. Use standard /review instead."
