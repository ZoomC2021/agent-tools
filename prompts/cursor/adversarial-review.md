...
3. Spawn at least two fresh read-only subagents in parallel when available.
4. Ask each subagent to review only the diff, read full files for context, and report `severity | category | file:line | issue | rationale | confidence | suggested fix`, or `NO_ISSUES`.
