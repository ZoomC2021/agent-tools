---
name: ce-strategy
description: "Create or update STRATEGY.md. Use when starting a product, changing direction or roadmap, or when product/planning workflows need upstream product grounding."
...
1. **Anchor, not plan.** Strategy is what the product is and why. Features belong in requirements work; schedules belong in the issue tracker. Do not let either creep into the doc.
...
After writing, note in one line where the file lives and that product and planning workflows should use it as grounding on their next run.

If no downstream product workflow has run yet on this repo, suggest `to-prd` or a focused requirements discussion as a next step.
...
- Does not write product requirements or implementation plans - those belong in `to-prd` or the host's planning workflow.
