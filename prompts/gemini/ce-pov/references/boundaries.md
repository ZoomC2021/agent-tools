...
| Options invented from an open field | Focused brainstorming / `to-prd` | `ce-pov` judges/selects from a discoverable field; it should not invent the whole field |
| To scope an idea already chosen | `to-prd` or focused requirements discussion | `ce-pov` decides *whether*; requirements work scopes *what* once it's a yes |
| To know how to build something decided | Host planning workflow | Verdict accepted → hand off; `ce-pov` does no task breakdown |
| To fix observed broken behavior | `diagnose` | `ce-pov` assesses *exposure and priority* of a CVE; diagnose investigates an *actual failure* |
...
- Field too open to enumerate → Hold → ask for a shortlist or run a focused brainstorming/product-discovery workflow → offer to re-run `ce-pov` on the shortlist.
- Criteria unclear / unstated requirements → Hold → use `to-prd` or a focused requirements discussion to surface them → offer to re-run.
