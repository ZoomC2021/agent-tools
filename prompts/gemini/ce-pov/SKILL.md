...
- **Adopt**, scope clear → offer to create an implementation plan using the host's normal planning workflow.
- **Adopt**, scope still fuzzy → offer `to-prd` or a focused requirements discussion before planning.
- **Trial** → scope a timeboxed spike with explicit success criteria.
