...
#   scripts/codex-ccapi.sh --model gpt-5.4
#
...
BASE_URL="${CCAPI_BASE_URL:-${OPENAI_BASE_URL:-https://api-direct.ccapi.us/v1}}"
...
Environment:
  ...
  CCAPI_BASE_URL      Default: https://api-direct.ccapi.us/v1
  OPENAI_BASE_URL     Backward-compatible alias for CCAPI_BASE_URL.

Examples:
  scripts/codex-ccapi.sh
  scripts/codex-ccapi.sh "Reply with exactly: OK"
  OPENAI_MODEL=gpt-5.5 scripts/codex-ccapi.sh
  OPENAI_MODEL=gpt-5.4 scripts/codex-ccapi.sh
...
if [[ -n "${CCAPI_API_KEY:-}" ]]; then
    export CCAPI_API_KEY="$CCAPI_API_KEY"
elif [[ -n "${OPENAI_API_KEY:-}" ]]; then
    export CCAPI_API_KEY="$OPENAI_API_KEY"
else
...
exec "$CODEX_BIN" \
    ...
    -c "model_providers.ccapi={name=\"ccapi\",base_url=\"$BASE_URL\",env_key=\"CCAPI_API_KEY\",wire_api=\"responses\"}" \
